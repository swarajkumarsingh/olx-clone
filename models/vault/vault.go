...
func Get(context context.Context, key string) (Struct, error) {
	...
	err := database.GetContext(context, &keyValueData, "SELECT key, value, created_at, updated_at FROM vault where key = $1", key)
...
func Create(context context.Context, key string, value string, tx *sqlx.Tx) (*Struct, error) {
	...
	query := `INSERT INTO vault (key, value, updated_at)
	VALUES (:key, :value, :updated_at) ON CONFLICT (key) DO UPDATE SET value = :value, updated_at = :updated_at;`
	if tx != nil {
